...
import WebGLController from '../managers/webgl-controller';
import { isMobile as mobileTest } from '../utils/utils';
import { EVENTS, VISIBLE_DATA_COUNT } from './constants';
import Loader, { getData } from '../utils/loader';
import EventManager from '../managers/event-manager';
import { AppProps, setAppProps } from './app-props';
import { showFallback } from '../managers/fallback';
import mapImage from "../mapImage.json"
...
export default class WebGLHeader {
  ...
  loadAssets() {
    ...
    const manifest = [
      { url: mapImage.mapImageData, id: 'worldMap' }
    ];
        ...
        .then((assets) => {
          ...
          this.loadData()
            .then((data) => {
              ...
              document.dispatchEvent(new CustomEvent('globeLoaded', { detail: { globeContainer: parentNode } }));
