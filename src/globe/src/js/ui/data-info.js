...
import { POPUP_TYPES } from '../core/constants';
...
export default class DataInfo {
  ...
  relativeTime(time) {
    // Custom relative time webcomponent from @github/relative-time-element
    const isoTimeString = new Date(time).toISOString();
    return `<relative-time tense="past" datetime="${isoTimeString}">${isoTimeString}</relative-time>`;
  }
